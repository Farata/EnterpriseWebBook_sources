define(["loadHtmlContent"], function (contentLoader) {

    var dataUrl = 'assets/json-data/chartdata.json';
    ...
    contentLoader.loadJsonContent(dataUrl, function (jsonData) {
        ...
        var labels = [];
        for (i = 0; i < chartData.items.length; i++) {
            labels.push(chartData.items[i].location);
        }

        drawPieChart(svgContainer, data, 50, 52, 50, colorScheme, labels, 115, 10);
    });
