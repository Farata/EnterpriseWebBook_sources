...
var pictureSource;
var destinationType;
var uploadedImagesPage = "http://k2fx.com/ssc-test/uploaded-images.php";
var photo;

function capturePhoto() {
	// Take picture using device camera and retrieve image as base64-encoded string
	navigator.camera.getPicture(onPhotoDataSuccess, onCapturePhotoFail, {
                                quality : 49,
...
function onCapturePhotoFail(message) {
	alert('Capture photo failed because: ' + message);
}
...
function uploadPhoto(imageURI) {
	...
	fileTransfer.upload(imageURI, "http://k2fx.com/ssc-test/upload.php", onUploadSuccess, onUploadFail, uploadOptions);
	...
	fileTransfer.onprogress = function(progressEvent) {
		if (progressEvent.lengthComputable) {
			...
			//console.log(uploadedPercentageMsg);
		} else {
...
function onDeviceReady() {
	...
	// --- Phonegap issue --- above code not working
	// with Phonegap for the first page in a document on 'pagebeforeshow' or
	// 'pageshow' jQM events
	...
	$(document).on("pageshow", "#Events", function() {
		...
		function resizemap() {
			var height = ($(window).height() - $("#Events").find('[data-role="header"]').outerHeight() - $("#Events").find('[data-role="footer"]').outerHeight());
